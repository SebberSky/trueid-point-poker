...
#   /        → portal :5170
#   /office  → office Vite (upstream includes /office)
#   /poker   → poker Vite (upstream includes /poker)
set -euo pipefail

PORTAL_UPSTREAM="${PORTAL_UPSTREAM:-http://127.0.0.1:5170}"
...
echo "==> Funnel / → portal ${PORTAL_UPSTREAM}"
tailscale funnel --bg --set-path=/ "${PORTAL_UPSTREAM}"
...
echo
echo "URLs:"
echo "  portal   https://agent3s-imac.taildc5084.ts.net/"
...
echo "Ensure pm2 apps trueid-portal + trueid-point-poker (+ office) are running."
